...
class GatekeeperController @Inject()(val authConnector: AuthConnector, val applicationService: ApplicationService,
  ...
  def approveUplift(id: UUID) = authenticated().async(parse.json) {
    implicit request =>
  ...
  def rejectUplift(id: UUID) = authenticated().async(parse.json) {
    implicit request =>
  ...
  def resendVerification(id: UUID) = authenticated().async(parse.json) {
    implicit request =>
  ...
  def deleteApplication(id: UUID) = authenticated().async(parse.json) {
    implicit request =>
  ...
  def blockApplication(id: UUID) = authenticated().async { implicit request =>
    gatekeeperService.blockApplication(id) map {
      case Blocked => Ok
    } recover recovery
  }

  def unblockApplication(id: UUID) = authenticated().async { implicit request =>
    gatekeeperService.unblockApplication(id) map {
      case Unblocked => Ok
    } recover recovery
  }

  def fetchAppsForGatekeeper = authenticated().async {
    gatekeeperService.fetchNonTestingAppsWithSubmittedDate() map {
      apps => Ok(Json.toJson(apps))
    } recover recovery
  }

  def fetchAppById(id: UUID) = authenticated().async {
